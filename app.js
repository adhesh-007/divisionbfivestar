...
function renderAreaDashboard(){
  ...
  const strengthBars = areaClubs.map(c=>({
    label: c.name.length>16 ? c.name.slice(0,14)+'…' : c.name,
    value: state.meetings.filter(m=>m.club===c.name).length
  }));
    ...
    <div class="panel">
      ...
      ${areaClubs.length ? `<table><thead><tr><th>Club</th><th>Meetings</th><th>5-Star Rate</th><th>Pathways</th><th>Mentor</th><th>Status</th></tr></thead><tbody>
        ${areaClubs.map(c=>{
          ...
          const pw = clubPathwaysPct(c.name); const mt = clubMentorPct(c.name);
          const st = computeClubStatus(c.name);
          return `<tr>
            <td>${escapeHtml(c.name)}</td><td>${count}</td>
            <td>${rate===null?'—':rate+'%'}</td><td>${pw===null?'—':pw+'%'}</td><td>${mt===null?'—':mt+'%'}</td>
            <td><span class="pill ${statusPillClass(st)}">${st}</span></td>
          </tr>`;
        }).join('')}
